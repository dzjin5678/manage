                        ...
                        <div class="card">
                            ...
                            <div class="content table-responsive table-full-width">
                                <table class="table table-hover table-striped">
                                    ...
                                    <td><h5>启泵时间</h5></td>
                                    <td><h5>停泵时间</h5></td>
                                    <td><h5>断开输油臂/软管时间</h5></td>
                                    <td><h5>离泊时间</h5></td>
                                    <td><h5>装卸时间</h5></td>
                                    <td><h5>装卸重量</h5></td>
                                    <td><h5>装卸体积</h5></td>
                                    <td><h5>装卸速度</h5></td>
                                    <td><h5>Z-T(在泊位时间)</h5></td>
                                    <td><h5>备注</h5></td>
                                    <td><h5>操作时间</h5></td>
                                    <td><h5>操作员</h5></td>
                                    <td><h5>操作</h5></td>
								...
								<c:forEach items="${shipOperationList}" var="pg" >
								<tr>
                                        <td>${pg.number}</td>
                                        <td>${pg.operation}</td>
                                        <td>${pg.connectionTime}</td>
                                        <td>${pg.startPump}</td>
                                        <td>${pg.stopTime}</td>
                                        <td>${pg.disconnectTime}</td>
                                        <td>${pg.unberthingTime}</td>
                                        <td>${pg.operationTime}</td>
                                        <td>${pg.weight}</td>
                                        <td>${pg.volume}</td>
                                        <td>${pg.speed}</td>
                                        <td>${pg.inBerthTime}</td>
                                        <td>${pg.remarks}</td>
                                        <td>${pg.time}</td>
                                        <td>${pg.realName}</td>
                                        <td><a href="/manage/tack/getUpdateShipOperationPage?id=${pg.id}&page=${page}&strip=10">更新</a></td>
                                        <td><a href="javascript:deleteShipOperation('${pg.id}');">删除</a></td>
                                </tr>
								</c:forEach>
                                    </tbody>
                                </table>
...
<script>
    ...
    function deleteShipOperation(id) {
        	var r=confirm("删除将无法找回！确定删除？");
        	if (r==true)
        	  {
        		$.ajax({
        	        type: "post",
        	        url: "/manage/tack/deleteShipOperation",
        	        datatype: "json",
        	        async: false,
        	        data:{
        	            id:id
