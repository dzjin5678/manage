                        ...
                        <div class="card">
                             <div class="header">
                                <div class="row">
                                    ...
                                    <div class="col-md-3" >
                                    <br>
                                    <a class="btn btn-primary btn-fill" href="/manage/tack/getAddGoodsPage?forecastId=${forecastId}">添加作业信息</a>
                            		</div>
                                </div>
                            </div>
                            <div class="row">
                            ...
                            <div class="content table-responsive table-full-width">
								...
								<c:forEach items="${goodsList}" var="pg" >
								<tr>
                                        <td>${pg.number}</td>
                                        <td>${pg.shipper}</td>
                                        <td>${pg.freightForwarding}</td>
                                        <td>${pg.carryOrder}</td>
                                        <td>${pg.goodsName}</td>
                                        <td>${pg.goodType}</td>
                                        <td>${pg.weight}</td>
                                        <td>${pg.volume}</td>
                                        <td>${pg.uninstallWeight}</td>
                                        <td>${pg.uninstallVolume}</td>
                                        <td>${pg.signName}</td>
                                        <td>${pg.releaseTime}</td>
                                        <td>${pg.extractDate}</td>
                                        <td>${pg.remarks}</td>
                                        <td>${pg.realName}</td>
                                        <td><a href="/manage/tack/getUpdateGoodsPage?id=${pg.id}&page=${page}&strip=10">更新</a></td>
                                        <c:if test="${pg.state=='0'}">
                                        <td><a onclick="clickFile('${pg.id}')">上传文件</a></td>
                                        </c:if>
                                        <c:if test="${pg.state=='1'}">
                                        <td>海关放行</td>
                                        </c:if>
                                        <c:if test="${pg.state=='2'}">
                                        <td>可放行</td>
                                        </c:if>
                                        <c:if test="${pg.filePath!=''}">
                                        <td><a href="/manage/upFile/${pg.filePath}" target="_Blank">查看海关文件</a></td>
                                        </c:if>
                                        <c:if test="${pg.filePath==''}">
                                        <td>待上传</td>
                                        </c:if>
                                        
                                        <td><a href="/manage/tack/getGoodsTaskList?goodsId=${pg.id}&page=1&strip=10">查看货物作业</a></td>
                                        <td><a href="javascript:deleteGoods('${pg.id}');">删除</a></td>
                                </tr>
								</c:forEach>
                                    </tbody>
                                </table>
                                
										<form action="http://127.0.0.1/manage/xmlFile/upload" id="uploadForm"
										      method="post" enctype="multipart/form-data" style="display: none" >
										    <input type="file" name="file" onchange="upFile()" id="upload"/>
										    <input type="text" name="upFileGoodsId" id="upFileGoodsId" />
...
<script>
    ...
    function deleteGoods(id) {
        ...
        function clickFile(id) {
            document.getElementById("upFileGoodsId").value=id;
