                        ...
                        <div class="card">
                             <div class="header">
                                <div class="row">
                                    ...
                                    <div class="col-md-3" >
                                    <br>
                                    <a class="btn btn-primary btn-fill" href="/manage/tack/getAddGoodsTaskPage?goodsId=${goodsId}">添加货物作业信息</a>
                            		</div>
                                </div>
                            </div>
                            <div class="row">
                            ...
                            <div class="content table-responsive table-full-width">
								...
								<c:forEach items="${goodsTaskList}" var="pg" >
								<tr>
                                        <td>${pg.stream}</td>
                                        <td>${pg.shift}</td>
                                        <td>${pg.reservoirArea}</td>
                                        <td>${pg.operation}</td>
                                        <td>${pg.amount}</td>
                                        <td>${pg.startTime}</td>
                                        <td>${pg.stopTime}</td>
                                        <td>${pg.realName}</td>
                                        <td>${pg.time}</td>
                                        <td><a href="/manage/tack/getUpdateGoodsTaskPage?goodsTaskId=${pg.id}&page=${page}&strip=10">更新</a></td>
                                        <td><a href="javascript:deleteGoodsTask('${pg.id}');">删除</a></td>
                                </tr>
								</c:forEach>
                                    </tbody>
                                </table>
...
<script>
    ...
    function deleteGoodsTask(id) {
        	var r=confirm("删除将无法找回！确定删除？");
        	if (r==true)
        	  {
        		$.ajax({
        	        type: "post",
        	        url: "/manage/tack/deleteGoodsTask",
        	        datatype: "json",
        	        async: false,
        	        data:{
        	            id:id
        	        ...
        	        success:function (data) {
        	        	...
        	        	if(dat.expect){
        	        		window.location.href="/manage/tack/getGoodsTaskList?page=${page}&strip=10&goodsId=${goodsId}";
        	        	}else{
        	        		alert(dat.msg);
        	        	}
        	        },
