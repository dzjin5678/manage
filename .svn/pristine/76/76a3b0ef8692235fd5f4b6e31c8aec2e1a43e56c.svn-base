                    ...
                    <div class="col-md-12">
                        <div class="card">
                             <div class="header">
                                <div class="row">
                                    <div class="col-md-6">
                                        <h4 class="title">船舶外贸提单管理</h4>
                                    </div>
                                    
                                    <div class="col-md-3" >
                                    <br>
                                    <a class="btn btn-primary btn-fill" href="/manage/ship_foreign_trade_bill/insertbefore?shipInformationId=${shipInformationId }">添加船舶外贸提单</a>
                            		</div>
                                </div>
                            </div>
                            <div class="row">
                            ...
                            <div class="content table-responsive table-full-width">
                                <table class="table table-hover table-striped">
                                    ...
                                    <td><h5>船舶信息表ID</h5></td>
                                    <td><h5>提单号</h5></td>
                                    <td colspan=2><h5>操作</h5></td>
                                    
                                  	<tbody>
										<c:forEach items="${requestScope.list}" var="sft" >
										<tr>
		                                        <td>${sft.shipInformationId}</td>
		                                        <td>${sft.billNumber}</td>
		                                        <td><a href="#" onclick="deleteShipForeignTradeBill('${sft.id}')">删除</a></td>
		                                        <td><a href="/manage/ship_foreign_trade_bill/updatebefore?id=${sft.id}&shipInformationId=${sft.shipInformationId}">编辑</a></td>
		                                </tr>
										</c:forEach>
                                    </tbody>
                                </table>
...
<script>
	...
	function deleteShipForeignTradeBill(id) {
		var r=confirm("删除将无法找回！确定删除？");
    	if (r==true)
    	  {
    		$.ajax({
    	        type: "post",
    	        url: "/manage/ship_foreign_trade_bill/delete",
    	        datatype: "json",
    	        async: false,
    	        data:{
    	            id:id
    	        ...
    	        success:function (data) {
    	        	var dat=JSON.parse(data);
    	        		window.location.href="/manage/ship_foreign_trade_bill/list?page="+${page}+"&strip=10&shipInformationId="+${shipInformationId}; 
    	        	alert("删除成功");
    	        },
    	        error:function(res){
    	            alert("联网失败");
