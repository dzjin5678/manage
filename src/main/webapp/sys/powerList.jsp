                    ...
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <div class="content table-responsive table-full-width">
								...
								<c:forEach items="${getPowerList}" var="pg" >
								<tr>
                                        <c:if test="${pg.createtime=='0'}">
                                        <td><input type="checkbox" onclick="updatePower('${pg.id}','${roleId}')"></td>
                                        </c:if>
                                        <c:if test="${pg.createtime!='0'}">
                                        <td><input type="checkbox" checked="checked" onclick="updatePower('${pg.id}','${roleId}')"></td>
                                        </c:if>
                                        <td>${roleName}</td>
                                        <td>${pg.explains}</td>
                                        <c:if test="${pg.createtime=='0'}">
...
<script>
	...
	function updatePower(id,roleId) {
		$.ajax({
	        type: "post",
	        url: "/manage/system/updatePower",
	        datatype: "json",
	        async: false,
	        data:{
	            id:id,
	        ...
	        success:function (data) {
	        	...
	        	if(dat.expect){
	        		alert("更新成功")
	        		window.location.href="/manage/system/getPowerList?roleId=${roleId}&page=${page}&strip=10&roleName=${roleName}";
	        	}else{
	        		alert("更新失败");
	        	}
