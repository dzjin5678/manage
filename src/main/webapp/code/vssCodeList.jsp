                    ...
                    <div class="col-md-12">
                        <div class="card">
                             <div class="header">
                                <div class="row">
                                    <div class="col-md-6">
                                        <h4 class="title">船舶管理</h4>
                                    </div>
                                    
                                    <div class="col-md-3" >
                                    <br>
                                    <a class="btn btn-primary btn-fill" href="/manage/code/addVssCode.jsp">添加船舶信息</a>
                            		</div>
                                </div>
                            </div>
                            <div class="row">
                            ...
                            <div class="content table-responsive table-full-width">
                                <table class="table table-hover table-striped">
                                    ...
                                    <td><h5>更新</h5></td>
                                  	...
                                  	<tbody>
										<c:forEach items="${getVssList}" var="vss" >
										<tr>
		                                        <td>${vss.vssCode}</td>
		                                        <td>${vss.vssCname}</td>
		                                        <td>${vss.vssEname}</td>
		                                        <td>${vss.nationality}</td>
		                                        <td>${vss.caption}</td>
		                                        <td>${vss.width}</td>
		                                        <td>${vss.tonnage}</td>
		                                        <td>${vss.cargoType}</td>
		                                        <td>${vss.loadline}</td>
		                                        <td>${vss.distance_outfall_bow}</td>
		                                        <td>${vss.unitCode}</td>
		                                        <td>${vss.agency}</td>
		                                        <td>${vss.remark}</td>
		                                        <td><a href="#" onclick="deleteRole('${vss.id}')">删除</a></td>
		                                        <td><a href="#" onclick="updateVss('${vss.id}')">更新</a></td>
		                                </tr>
										</c:forEach>
                                    </tbody>
                                </table>
...
<script>
    	...
    	if (r==true)
    	  {
    		$.ajax({
    	        type: "post",
    	        url: "/manage/vssCode/deleteVssCode",
    	        datatype: "json",
    	        async: false,
    	        data:{
    	            id:id
    	        ...
    	        success:function (data) {
    	        		...
    	        		window.location.href="/manage/vssCode/queryVssCodeCode?page="+${page}+"&strip=10";
	...
	function updateVss(id) {

    		$.ajax({
    	        type: "post",
    	        url: "/manage/vssCode/getVssCode?id="+id,
    	        datatype: "json",
    	        async: false,
    	        data:{
    	            id:id
    	        ...
    	        success:function () {
    	        	window.location.href="/manage/code/updateVssCode.jsp";
    	        },
    	        error:function(res){
    	            alert("联网失败");
    	        }
