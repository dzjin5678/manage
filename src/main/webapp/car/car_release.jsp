                        ...
                        <div class="card">
                             <div class="header">
                                <div class="row">
                                    <div class="col-md-6">
                                    <c:if test="${empty type }">
                                        <h4 class="title">车辆放行管理</h4>
                                        </c:if>
                                        <c:if test="${type eq 'weigh' }">
                                        <h4 class="title">车辆过磅管理</h4>
                                        </c:if>
                                    </div>
                                    <c:if test="${empty type }">
                                        <div class="col-md-3" >
                                    <br>
                                    <a class="btn btn-primary btn-fill" href="/manage/car_release/editCarRelease?type=add">添加车辆放行信息</a>
                            		</div>
                                        </c:if>
                                     ...
                                     <td><h5>操作</h5></td>
                                  	<tbody>
										...
										<tr>
		                                        ...
		                                        <td>${sfc.handlingTypeStr}</td>
		                                        <td><fmt:formatDate value="${sfc.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
		                                        <td>${sfc.releaseTypeStr}</td>
		                                        <td>${sfc.releaseStartTime}</td>
		                                         <td>${sfc.releaseEndTime}</td>
		                                          <td>${sfc.cargotype}</td>
		                                           <td>${sfc.amount}</td>
		                                            <td>${sfc.isCancelStr}</td>
		                                       
		                                        <td>
		                                       
		                                        <c:if test="${empty type }">
		                                        	<c:if test="${sfc.isCancel eq 1 }">
		                                        <a href="#" onclick="deleteCarRelease('${sfc.id}')">删除</a>
		                                        </c:if>
		                                        <c:if test="${sfc.isCancel eq 0 }">
		                                        <a href="#" onclick="updateCarRelease('${sfc.id}')">取消</a>
		                                        </c:if>
		                                        <a href="/manage/car_release/editCarRelease?type=update&id=${sfc.id }" >编辑</a>
                                        		</c:if>
                                        		<c:if test="${type eq 'weigh' }">
                                        			<a href="/manage/car_weight/editCarWeigh?type=add&releaseId=${sfc.id }" >添加过磅信息</a>
		                                        	<a href="/manage/car_weight/weightlist?releaseId=${sfc.id }" >查看过磅信息</a>
                                        		</c:if>
                                        		 </td>
		                                </tr>
										</c:forEach>
...
<script>
	...
	function updateCarRelease(id) {
		var r=confirm("确定取消？");
    	if (r==true)
    	  {
    		$.ajax({
    	        type: "post",
    	        url: "/manage/car_release/cancel",
    	        datatype: "json",
    	        async: false,
    	        data:{
    	            id:id
